...
def get_draft_statuses():
    ...
    :return: (DataFrame)
        ...
        champion_id     | (int)
        role            | (int)
        banned_champs   | (list[int]) champion_id
        our_champs      | (list[int]) champion_id * 5 + role
        other_champs    | (list[int]) champion_id * 5 + role
        win             | (bool)
    """
    ...
    #     draft_statuses[BANNED_CHAMPS] = [str(x)[1:-1] for x in draft_statuses[BANNED_CHAMPS].values]
    #     draft_statuses[BANNED_CHAMPS] = [x.split(', ') for x in draft_statuses[BANNED_CHAMPS]]
    #     draft_statuses[BANNED_CHAMPS] = [[int(y) for y in x] for x in draft_statuses[BANNED_CHAMPS]]
    #
    #     draft_statuses[OUR_CHAMPS] = [str(x)[1:-1] for x in draft_statuses[OUR_CHAMPS].values]
    #     draft_statuses[OUR_CHAMPS] = [x.split(', ') for x in draft_statuses[OUR_CHAMPS]]
    #     draft_statuses[OUR_CHAMPS] = [[int(y) for y in x] for x in draft_statuses[OUR_CHAMPS]]
    #
    #     draft_statuses[OTHER_CHAMPS] = [str(x)[1:-1] for x in draft_statuses[OTHER_CHAMPS].values]
    #     draft_statuses[OTHER_CHAMPS] = [x.split(', ') for x in draft_statuses[OTHER_CHAMPS]]
    #     draft_statuses[OTHER_CHAMPS] = [[int(y) for y in x] for x in draft_statuses[OTHER_CHAMPS]]
    # else:
    ...
    pick_67.loc[:, PICK_67] = pd.Series([list(tup) for tup in zip(pick_67.loc[:, GB_6], pick_67.loc[:, GB_7])])
    ...
    pick_123.loc[:, PICK_123] = pd.Series([list(tup) for tup in zip(pick_123.loc[:, GB_1], pick_123.loc[:, GB_2], pick_123.loc[:, GB_3])])
    ...
    pick_6789.loc[:, PICK_6789] = pd.Series([list(tup) for tup in zip(pick_6789.loc[:, GB_6], pick_6789.loc[:, GB_7], pick_6789.loc[:, GB_8], pick_6789.loc[:, GB_9])])
    ...
    pick_12345.loc[:, PICK_12345] = pd.Series([list(tup) for tup in zip(pick_12345.loc[:, GB_1], pick_12345.loc[:, GB_2], pick_12345.loc[:, GB_3], pick_12345.loc[:, GB_4], pick_12345.loc[:, GB_5])])
    ...
    final_df[OUR_CHAMPS] = final_df.apply(lambda x: [], axis=1)
    final_df[OTHER_CHAMPS] = final_df.apply(lambda x: [], axis=1)
    ...
    final_df = final_df.join(aggregated, how='inner', on=MATCH_ID)
    ...
    # draft_statuses = draft_statuses.iloc[np.where(draft_statuses.notnull())]
